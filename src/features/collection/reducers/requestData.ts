...
function reducer(state: State = initialState, action) {
  switch (action.type) {
    ...
    case actionTypes.SET_PAGE: return handleSetPage(state, action.payload);
    case actionTypes.SET_SORTING: return handleSetSorting(state, action.payload);
    case actionTypes.UNSET_SORTING: return handleUnsetSorting(state, action.payload);
    case actionTypes.SET_NESTED_LIST_FACET: return handleSetNestedListFacet(state, action.payload);
    case actionTypes.UNSET_NESTED_LIST_FACET: return handleUnsetNestedListFacet(state, action.payload);
    case actionTypes.SET_TEXT_FACET: return handleSetTextFacet(state, action.payload);
    case actionTypes.UNSET_TEXT_FACET: return handleUnsetTextFacet(state, action.payload);
    case actionTypes.SET_RANGE_FACET: return handleSetRangeFacet(state, action.payload);
    case actionTypes.UNSET_RANGE_FACET: return handleUnsetRangeFacet(state, action.payload);
...
function handleSetNestedListFacet(state, { name, value }) {
  ...
  const isItemExists = filters.filter((f) => f.name === name && f.type === 'category').length > 0;
  const item = {
    type: 'category',
    name,
    values: [{ value }],
...
function handleSetTextFacet(state, { name, value }) {
  const isNeededFilter = (filter) => (
    filter.name === name && filter.type === 'text' && (
      !filter.values || filter.values.filter((item) => item.value === value).length === 0
    )
  );
  const filters = state.filters || [];
  const isItemExists = filters.filter((f) => f.name === name && f.type === 'text').length > 0;
  const item = {
    type: 'text',
    name,
    values: [{ value }],
  ...
  const nextState = {
    ...
    filters: filters.map((el) => (
      el.name === name && el.type === 'text' ? ({
        ...el,
        values: el.values.filter((v) => !(v.value === value)),
      }) : (
        el
      )
    )).filter((el) => (
...
function handleSetRangeFacet(state, { name, from, to }) {
  const isNeededFilter = (filter) => (
    filter.name === name && filter.type === 'range' && (
      !filter.values || filter.values.filter((item) => item.from === from && item.to === to).length === 0
    )
  );
  const filters = state.filters || [];
  const isItemExists = filters.filter((f) => f.name === name && f.type === 'range').length > 0;
  const item = {
    type: 'range',
    name,
    values: [cleanObject({
  ...
  const nextState = {
    ...
    filters: filters.map((el) => (
      el.name === name && el.type === 'range' ? ({
        ...el,
        values: el.values.filter((v) => !(v.from === from && v.to === to)),
      }) : (
        el
      )
    )).filter((el) => (
